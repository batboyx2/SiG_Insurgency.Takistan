westVehicleChecker = {
    ...
    if (!alive MHQ && startLocation == MHQ) exitWith {
		...
		//processInitCommands;
		[[_vcl, "MHQ"], "setVehicleVarName", true, true] call BIS_fnc_MP;
	...
	if (getMarkerPos "USFLAG" distance (HQ modelToWorld [0,0,0]) > 50) then {
		baseSpawn setPosATL _pos;
		baseSpawn setDir (getDir startLocation);
		baseSpawn setCaptive true;
	};
...
resetWestVehicle = {
	...
	if _bool then {
		...
		if (_vclTimeDiff <= westVehicleRespawn && _vclKilledTime != 0) exitWith {};
...
checkVehiclesNearbyCampObjs	= {
	...
	if (_isemptycar && _vcl distance (baseSpawn modelToWorld [0,0,0]) < 2) then {
