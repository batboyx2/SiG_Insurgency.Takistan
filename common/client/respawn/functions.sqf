...
respawnSystem = {
	...
	while {isDead(player) && time > 30} do {	
		if (isNull camPlayer || _camPlayer != camPlayer) then {
			_arr = groupMembers(true,"array")+[baseSpawn];
			...
			if (camPlayer == baseSpawn) then { camAttachCoords = [0, 0, 12]; };			
			// respawnCamera camSetTarget camPlayer;
			respawnCamera camSetTarget fobCam;
		...
		if (_camPlayer == camPlayer && enterSpawn) then {
			...
			if (camPlayer != baseSpawn && (nearestEastMen(getPosATL camPlayer,respawnRange,true,"count") > 0
