gridPath = {
	...
	if isNil "_gMkr" exitWith {diag_log "PROBLEM_line 6"};
	if (markerColor _gMkr == "ColorGreen") exitWith {diag_log "PROBLEM_line 7"};
	if (nearestEastMen(call compile _gMkr,70,true,"count") > 0 
	|| nearestPlayers(call compile _gMkr,140,true,"count") < playersNeeded) exitWith {diag_log "PROBLEM_line 8, 9"};
